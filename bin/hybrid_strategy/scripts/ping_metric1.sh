...
while read -r ip; do
    ...
    while read -r dest_ip; do
        ...
        ping_metrics=$(ping -c 5 -i 0.2 -q "$dest_ip" | tail -n 2 | head -1)
        ping_metric_rtt=$(ping -c 5 -i 0.2 -q "$dest_ip" | tail -n 2 | tail -1)
        ...
        echo "$name,$dest_ip,$dest_name,$cpu_utilization" >> "${name}_cpu.csv"
    done < <(grep -v "$ipa" "$ip_file" | awk -F',' '{print $1}')

    # Ping the IP address of the machine itself and capture the metrics
    ping_metrics=$(ping -c 5 -i 0.2 -q "$ipa" | tail -n 2 | head -1)
    ping_metric_rtt=$(ping -c 5 -i 0.2 -q "$ipa" | tail -n 2 | tail -1)
...
# Combine all the CPU utilization CSV files into a single file and separate lines with the source machine name
echo > $cpu_output_file
for f in *_cpu.csv; do
    echo "${f%%_*}"
