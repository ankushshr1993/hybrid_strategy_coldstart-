...
while read -r ip; do
    ...
    # Capture the CPU usage information using top command
    ssh -oStrictHostKeyChecking=no -i /path/to/private/key "$ipa" "top -b -n 1 | grep Cpu" > "${name}_cpu.csv"

    # Write the metrics to the output file with the name of the source machine
    echo "$name,$ipa,$name,$(<"${name}_cpu.csv"),${ping_metric_rtt}" >> "${name}_cpu.csv"
